...
<div class="box box-{{ $errors->any() ? 'danger' : 'success' }}">
    ...
    @endif
            ...
            <div class="form-group {{ $errors->has('allow_operator') ? 'has-error' : '' }}">
                ...
                @foreach ($operator as $key => $val)
                    <option value="{{ $key }}" {{ in_array($key, old('allow_operator',$row['allow_operator'])) ? 'selected' : '' }}>{{ $val }}</option>
                @endforeach
                </select>
                {!! $errors->first('allow_operator', '<span class="help-block">:message</span>') !!}
            </div>
