            ...
            <div class="col-xs-1" style="width:auto;">
                ...
                <select name="operator" class="form-control">
                    ...
                    @foreach ($operator as $key => $val)
                        <option value="{{ $key }}" {{ $search['operator'] ?? -1 == $key ? 'selected' : '' }}>{{ $val }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-xs-1">
        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th>{!! sort_title('allow_operator', '运营商权限', $route, $order, $search) !!}</th>
                    <th>{!! sort_title('create_time', '添加日期', $route, $order, $search) !!}</th>
                    <th>{!! sort_title('update_time', '修改日期', $route, $order, $search) !!}</th>
